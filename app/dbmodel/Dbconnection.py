...
def create_items(container,document):
...
def read_item(container, doc_id, partition_id):
    print('\n1.2 Reading Item by Id\n')

    # We can do an efficient point read lookup on partition key and id
    response = container.read_item(item=doc_id, partition_key=partition_id)
...
def initContainer(self,partition_path):
    ...
    try:
        ...
        try:
            ...
            container = db.create_container_if_not_exists(id=CONTAINER_ID, partition_key=PartitionKey(path=partition_path),
...
class DbConnection :
    ...
    def __init__(self):
        self.container = initContainer(self, self.path)

    def save(self,container, document):
        if container:
            try:
                return create_items(container, document)
            except Exception as e:
                print(e)
        return

    def readByDoc(self,doc_id):
        return read_item(self.container, doc_id, self.path)
    def readAllDoc(self):
        return read_items(self.container)
